app.controller('AppController', ['$rootScope', '$scope', 'SharedData', function($rootScope, $scope, SharedData) {
    ...
    lb.on(ev[0], function() {
        ...
        lb.on(ev[2], function(round) {
            ...
            $scope.getRoundData([round]);
            $scope.getPlayerData([round]);
              ...
              && storage.getItem("_" + kRId) != "undefined")
            {
                lb.on(ev[4], function(score) {
                    $scope.getScoreData(score);
                    $scope.setTeamScoreData();
                });
    
                // スコア要求
                $scope.requestPersonalScore(lb, storage.getItem("_" + kRId), 2);
            }
        });
    });
    ...
    $scope.$on('requestRoundInfo', function(event, data) {
        console.log('scope.on:requestRoundInfo');
        $scope.requestRoundInfo(data);
    });

    /******************************************************************
     * ラウンド情報要求
     ******************************************************************/
    $scope.requestRoundInfo = function( rid ) {
     ...
     * ラウンド情報の取得
     ******************************************************************/
    $scope.getRoundData = function( roundData ) {
    	// ラウンドID
    ...
    */
     ...
     ******************************************************************/
    $scope.getPlayerData = function( roundData ) {
    	var arrayPlayers = [];
    	var arrayTeams = [];
    	var k = 0;
    	var l = 0;
     ...
     * スコア要求（初回接続時　サーバへの送信）
     ******************************************************************/
    $scope.requestPersonalScore = function( lb, rid, type ) {
     ...
     * スコア情報取得処理（初回接続時）
     ******************************************************************/
    $scope.getScoreData = function( scoreData )
    {
    	// 各プレイヤースコア
     ...
     * スコア情報取得処理（初回接続時）
     ******************************************************************/
    $scope.setTeamScoreData = function() {
     ...
     ******************************************************************/
    $scope.getCulculateAge = function(birthday) {
    	// yyyy-MM-ddのみ取得
